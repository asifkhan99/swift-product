...
/** class DataSource: JSONRepresentable {
    ...
    let source = [Employee(email: "test@test.org", name: "test").self]

    func makeJSON() throws -> JSON {
        let content = DataSource().source.map { $0.makeJson() }
        return JSON.object(["content": JSON.array(content)])
    }

}

drop.get("employee") { request in
  return DataSource().makeJSON()
} */

drop.get("emergency") { request in
